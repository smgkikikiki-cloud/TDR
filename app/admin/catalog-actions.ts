 ...
 * automotive/. Keeping this export makes stale clients fail closed instead of
 * silently writing a second vehicle master into the editorial tables.
 */
export async function saveModelV12() {
  await requireAdmin();
  throw new Error("Vehicle facts are read-only here. Edit automotive/Vehicle Master and publish one canonical release.");
}
