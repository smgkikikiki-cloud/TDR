  ...
  return <article className={styles.card}>
    <Link href={`/models/${model.slug}`} className={styles.visualLink}>
      {model.imageUrl
        ...
        : <div className={styles.placeholder}><small>{model.brand.toUpperCase()}</small><b>{model.name}</b></div>}
    ...
    <div className={styles.cardBody}>
      ...
      {model.priceLabel
        ...
        : <div className={styles.priceMissing}>{model.verifiedCurrent ? "ยังไม่ประกาศราคา" : "ยังไม่แสดงราคาปัจจุบัน"}</div>}
  ...
  return <>
    ...
    {models.length ? <section className={styles.bridge}>
      <div className={styles.bridgeCopy}>
        ...
        <div><b>อยากรู้ว่ารุ่นไหนนำตลาดในกลุ่มนี้?</b><p>ดูยอดจดทะเบียน ส่วนแบ่งตลาด และแนวโน้มแบบเจาะลึกด้วย Market Intelligence</p></div>
      </div>
    ...
    {selected.length ? <aside className={styles.tray} aria-label="รถที่เลือกไว้เปรียบเทียบ">
      <div className={styles.trayInner}>
        ...
        <div className={styles.trayModels}>
          {selected.map((id) => {
            const model = byId.get(id);
            if (!model) return null;
            return <div className={styles.trayModel} key={id}>
              {model.imageUrl ? <img className={styles.trayThumb} src={model.imageUrl} alt="" /> : null}
              <span>{model.brand} {model.name}</span>
              <button type="button" aria-label={`เอา ${model.name} ออกจากรายการ`} onClick={() => toggle(id)}>×</button>
            </div>;
          })}
