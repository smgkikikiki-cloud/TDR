...
const inputPipeline = fs.readFileSync("automotive/vehicle_master/vehreg/input_pipeline.py", "utf8");
const reviewWriter = fs.readFileSync("automotive/vehicle_master/vehreg/eco_review_write.py", "utf8");
...
check("create action checks duplicate trim identity", action.includes("duplicateIdentity"), true);
check("existing attach action exists", action.includes("export async function enqueueEcoAttachExistingTrim"), true);
check("attach action requires exact target generation", action.includes("MarketTrim ปลายทางอยู่คนละ generation"), true);
check("attach action requires exact powertrain", action.includes("MarketTrim ปลายทางมี powertrain ไม่ตรง ECO candidate"), true);
check("attach path targets canonical trim id instead of creating another identity", action.includes("canonical_id: targetTrim.canonical_id"), true);
check("attach path merges source refs instead of replacing unrelated evidence", action.includes("mergedSourceRefs(targetTrim.source_refs, context.group.sourceIds)"), true);
check("review disposition uses the canonical input queue", action.includes('operation: "UPSERT_ECO_REVIEW"'), true);
check("browser action never supplies reviewer identity", action.includes("reviewer:"), false);
check("review disposition supports reject defer and reopen", action.includes('["reject", "defer", "reopen"]'), true);
check("special review command requires ECO source kind", inputPipeline.includes('source_kind != "ECO"'), true);
check("special review command rejects agent actor", inputPipeline.includes('"agent-proposed"'), true);
check("review writer preserves unrelated decisions", reviewWriter.includes('decision["source_id"] not in selected'), true);
check("review writer supports reopening by removing selected decisions", reviewWriter.includes('if action != "reopen"'), true);
...
check("page exposes attach to existing as an explicit HUMAN decision", page.includes("Attach evidence to existing"), true);
check("page only offers existing trims with same model generation and powertrain", page.includes("row.generation_id === group.generationId") && page.includes("=== group.powertrain"), true);
check("page loads versioned ECO review state", page.includes("review/2026-09-08.json"), true);
