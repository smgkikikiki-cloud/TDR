...
check("reopen is not blocked by stale parent lifecycle", action.includes('action !== "reopen" && canonicalModelStatus !== "CURRENT"'), true);
...
check("workflow store exempts reopen from parent CURRENT guard", reviewStore.includes('if action != "reopen" && _parent_model_status'), true);
