...
check("legacy lowercase current is not accepted as lifecycle evidence", helper.includes('return value === "CURRENT"'), true);
check("model lifecycle comes from the enriched serving projection, CURRENT by default", helper.includes("model?.status") && helper.includes("CURRENT by default"), true);
