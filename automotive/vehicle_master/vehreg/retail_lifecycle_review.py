...
def upsert_trim_lifecycle_disposition(*, data_dir: Path | str = DATA_DIR,
    ...
    # A trim-level claim only makes sense while its canonical parent model is
    # explicitly CURRENT. Keep this invariant inside the workflow store so
    # advanced/admin batches cannot bypass the web action. Reopen is exempt:
    # stale sidecar state must remain removable after a parent becomes historical.
